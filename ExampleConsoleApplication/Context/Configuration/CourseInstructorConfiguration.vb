...
Namespace Context.Configuration
	Public Class CourseInstructorConfiguration
		...
		Public Sub Configure(builder As EntityTypeBuilder(Of CourseInstructor)) Implements IEntityTypeConfiguration(Of CourseInstructor).Configure
			...
			builder.HasOne(Function(d) d.Course).WithMany(Function(p) p.CourseInstructors).HasForeignKey(Function(d) d.CourseID).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_CourseInstructor_Course")
			builder.HasOne(Function(d) d.Person).WithMany(Function(p) p.CourseInstructors).HasForeignKey(Function(d) d.PersonID).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_CourseInstructor_Person")
		End Sub
	End Class
End Namespace
