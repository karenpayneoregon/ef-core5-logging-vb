...
Namespace Context.Configuration
	Public Class OfficeAssignmentConfiguration
		...
		Public Sub Configure(builder As EntityTypeBuilder(Of OfficeAssignment)) Implements IEntityTypeConfiguration(Of OfficeAssignment).Configure
			...
			builder.Property(Function(e) e.InstructorID).
				ValueGeneratedNever()

			builder.Property(Function(e) e.Location).
				IsRequired().
				HasMaxLength(50)

			builder.Property(Function(e) e.Timestamp).
				IsRequired().IsRowVersion().
				IsConcurrencyToken()

			builder.HasOne(Function(d) d.Instructor).
				WithOne(Function(p) p.OfficeAssignment).
				HasForeignKey(Of OfficeAssignment)(Function(d) d.InstructorID).
